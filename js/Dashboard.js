...
async function renderDashboardView() {
  ...
  if (!contentPanel) {
    console.error("Dashboard Error: Target container '#main-content' not found in DOM.");
    return;
  }
...
async function fetchDashboardMetricsData() {
  ...
  try {
    ...
    // Extract payload safely whether it comes wrapped in response.data or raw
    ...
    if (metricsData && typeof metricsData.totalStudents !== 'undefined') {
      ...
      throw new Error((response && response.message) || "Invalid data structure returned from server.");
    }
  } catch (err) {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', renderDashboardView);
} else {
  renderDashboardView();
}
